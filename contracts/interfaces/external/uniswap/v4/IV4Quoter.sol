...
import { PathKey } from
    "contracts/interfaces/external/uniswap/v4/libraries/PathKey.sol";
...
interface IV4Quoter is IImmutableState {
    ...
    /// @notice Returns the delta amounts for a given exact input swap of a
    /// single pool
    /// @param params The params for the quote, encoded as
    ...
    /// @notice Returns the delta amounts along the swap path for a given exact
    /// input swap
    /// @param params the params for the quote, encoded as 'QuoteExactParams'
    /// currencyIn The input currency of the swap
    /// path The path of the swap encoded as PathKeys that contains currency,
    /// fee, tickSpacing, and hook info
    /// exactAmount The desired input amount
    /// @return amountOut The output quote for the exactIn swap
    /// @return gasEstimate Estimated gas units used for the swap
    ...
    /// @notice Returns the delta amounts for a given exact output swap of a
    /// single pool
    /// @param params The params for the quote, encoded as
    /// `QuoteExactSingleParams`
    /// poolKey The key for identifying a V4 pool
