...
import { Position } from
    "contracts/interfaces/external/uniswap/v4/libraries/Position.sol";
...
library StateLibrary {
    ...
    /// @notice index of TicksInfo mapping in Pool.State: mapping(int24 =>
    /// TickInfo) ticks;
    ...
    /// @notice index of Position.State mapping in Pool.State: mapping(bytes32
        ...
        )
    {
        ...
        // 0x000000   |000bb8|000000      |ffff75
        // |0000000000000000fe3aa841ba359daa0ea9eff7
    ...
    function getTickLiquidity(
        ...
        PoolId poolId,
        int24 tick
    ) internal view returns (uint128 liquidityGross, int128 liquidityNet) {
        bytes32 slot = _getTickInfoSlot(poolId, tick);
        ...
        returns (uint256 feeGrowthOutside0X128, uint256 feeGrowthOutside1X128)
    {
        ...
        // offset by 1 word, since the first word is liquidityGross +
        // liquidityNet
        ...
        returns (uint256 feeGrowthGlobal0, uint256 feeGrowthGlobal1)
    {
        ...
        bytes32 slot_feeGrowthGlobal0X128 =
            bytes32(uint256(stateSlot) + FEE_GROWTH_GLOBAL0_OFFSET);
    ...
    function getTickBitmap(
        ...
        PoolId poolId,
        int16 tick
    ) internal view returns (uint256 tickBitmap) {
        // slot key of Pool.State value: `pools[poolId]`
        bytes32 stateSlot = _getPoolStateSlot(poolId);

        // Pool.State: `mapping(int16 => uint256) tickBitmap;`
        bytes32 tickBitmapMapping =
            bytes32(uint256(stateSlot) + TICK_BITMAP_OFFSET);

        // slot id of the mapping key: `pools[poolId].tickBitmap[tick]
        bytes32 slot =
            keccak256(abi.encodePacked(int256(tick), tickBitmapMapping));

        tickBitmap = uint256(manager.extsload(slot));
    }

    /**
     * @notice Retrieves the position information of a pool without needing to
     ...
     * gas efficient for just retrieiving liquidity as compared to
     * getPositionInfo
     * @param manager The pool manager contract.
     * @param poolId The ID of the pool.
     * @param positionId The ID of the position.
     ...
     * @dev pools[poolId].feeGrowthInside0LastX128 in Position.State is cached
    ...
    function _getPoolStateSlot(
        PoolId poolId
    ) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(PoolId.unwrap(poolId), POOLS_SLOT));
    }

    function _getTickInfoSlot(
        PoolId poolId,
        int24 tick
    ) internal pure returns (bytes32) {
        // slot key of Pool.State value: `pools[poolId]`
