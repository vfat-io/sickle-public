...
/// @notice Wraps Algebra positions in a non-fungible token interface which
...
interface IAlgebraNonfungiblePositionManager {
    ...
    /// @param liquidityDesired The amount by which liquidity for the NFT
    /// position was increased
    /// @param actualLiquidity the actual liquidity that was added into a pool.
    /// Could differ from
    ...
    /// @notice Emitted if farming failed in call from
    ...
    /// @dev If native token is used as input, this function should be
    /// accompanied by a `refundNativeToken` in multicall to avoid potential
    ...
    /// @param farmingAddress The address of farming: used to prevent tx
    /// frontrun
    ...
    /// @dev can be called only by factory owner or
    ...
    /// @return The address of the farming center contract, which handles
    /// farmings logic
    function farmingCenter() external view returns (address);

    /// @notice Returns the address of farming that is approved for this token,
    /// if any
    function farmingApprovals(
        uint256 tokenId
    ) external view returns (address);

    /// @notice Returns the address of farming in which this token is farmed, if
    /// any
    function tokenFarmedIn(
        uint256 tokenId
    ) external view returns (address);
}
