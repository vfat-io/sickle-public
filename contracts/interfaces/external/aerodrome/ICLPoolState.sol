...
interface ICLPoolState {
    ...
    /// @dev Can be modified in PoolFactory on a pool basis or upgraded to be
    /// dynamic.
    ...
    /// @notice The reward growth as a Q128.128 rewards of emission collected
    /// per unit of liquidity for the entire life of the pool
    /// @dev This value can overflow the uint256
    ...
    /// @notice acts as a virtual reserve that holds information on how many
    /// rewards are yet to be distributed
    ...
    /// @notice tracks total rewards distributed when no staked liquidity in
    /// active tick for epoch ending at periodFinish
    /// @notice this amount is rolled over on the next call to
    /// notifyRewardAmount
    /// @dev rollover will always be smaller than the rewards distributed that
    /// epoch
    ...
    /// @dev This value has no relationship to the total staked liquidity across
    /// all ticks
    ...
    /// RewardGrowthGlobalX128 can be supplied as a parameter for claimable
    /// reward calculations.
    /// @dev Used in gauge reward/earned calculations
    /// @param tickLower The lower tick of the range
    /// @param tickUpper The upper tick of the range
    /// @param _rewardGrowthGlobalX128 a calculated rewardGrowthGlobalX128 or 0
    /// (in case of 0 it means we use the rewardGrowthGlobalX128 from state)
