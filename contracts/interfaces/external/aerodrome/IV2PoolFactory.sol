...
interface IV2PoolFactory {
    ...
    /// @notice Support for v3-style pools which wraps around
    /// getPool(tokenA,tokenB,stable)
    ...
    /// @dev Only called once to set to Voter.sol - Voter does not have a
    /// function
    ///      to call this contract method, so once set it's immutable.
    ///      This also follows convention of setVoterAndDistributor() in
    /// VotingEscrow.sol
    /// @param _voter .
    function setVoter(
        address _voter
    ...
    /// @notice Support for v3-style pools which wraps around
    /// createPool(tokena,tokenB,stable)
    /// @dev fee is converted to stable boolean
    /// @dev token order does not matter
    /// @param tokenA .
    /// @param tokenB .
